...
1. **孔隙率公式**：
   ...
   \]
   式中，分子表示1g催化剂颗粒中孔的体积（比孔容），分母表示1g催化剂颗粒的体积【11:5†source】。
...
2. **平均孔半径**：
   ...
   \overline{r} = \frac{R}{3}
   \]
   其中，\( V_g \)为比孔容，\( S_g \)为比表面积【11:0†source】【11:3†source】。

3. **Kelvin公式**：
   \[
   r_k = -\frac{2\sigma V_1 \cos \phi}{RT \ln \left( \frac{p}{p_0} \right)}
   \]
   式中，\( r_k \)为孔半径，σ为吸附质液体的表面张力，\( V_1 \)为吸附质的摩尔体积，φ为接触角，p为吸附质的蒸气压力，\( p_0 \)为饱和蒸气压力【11:6†source】。
...
4. **校正的孔径尺寸**：
   ...
   \]
   t与p/p₀的关系可由经验式决定【11:2†source】【11:4†source】。

5. **催化剂的孔数**：
   \[
   n_p = \frac{\theta S_x S_g}{4 \pi V_g^2}
   \]
   其中，θ、\( S_x \)、\( V_g \)均可由实验测得【11:0†source】。

6. **催化剂颗粒的表面积**：
   \[
   m_p S_g = n_p (2 \pi r \overline{L})
   \]
   其中，\( m_p \)为颗粒的质量，\( S_g \)为比表面积，\( n_p \)为孔数，r为孔半径，\( \overline{L} \)为孔长度【11:1†source】。

7. **比孔体积**：
   \[
   V_{\text{孔}} = \frac{V_{\text{He}} - V_{\text{Hg}}}{W}
   \]
   其中，\( V_{\text{He}} \)和\( V_{\text{Hg}} \)分别为氦和汞置换体积，W为样品质量【11:5†source】。

8. **压汞法公式**：
   \[
   P = \frac{-2\sigma \cos \theta}{r}
   \]
   其中，P为汞压入孔隙所需的压力，σ为汞的表面张力，θ为汞与孔壁的接触角，r为孔隙半径【11:6†source】。

二. 笔记
1. **孔隙率**：催化剂的孔隙率表示每克催化剂内孔体积与颗粒体积之比，是表征催化剂孔结构的重要参数。计算公式为孔体积与颗粒体积之比【11:5†source】。

2. **平均孔半径与孔长**：实际催化剂颗粒中孔结构复杂无序，常用简化模型来计算平均孔半径和平均孔长。公式中通过比孔容和比表面积来计算平均孔径和孔长【11:0†source】【11:3†source】。

3. **孔隙分布曲线**：
   - **孔径分布积分曲线：**
      \[ \frac{V_1}{V_{\text{孔}}} \%_2 - \frac{V_1}{V_{\text{孔}}} \%_0 \]
      其中 \( \frac{V_1}{V_{\text{孔}}} \) 表示孔径范围 \( r_{p1} \) 到 \( r_{p2} \) 的孔体积百分数。

   - **孔径分布微分曲线：**
     将 \( \frac{\Delta V}{\Delta r_p} \) 对 \( r_p \) 作图，得到孔径分布微分曲线。峰值对应的孔径 \( r_m \) 为最可几孔半径。
...
4. **气体吸附法**：
   ...
   - **关键公式：**
     ...
     - **Langmuir等温线：**
       \[ \theta = \frac{bP}{1 + bP} \]
       其中 \( \theta \) 是表面覆盖度，\( P \) 是压力，\( b \) 是与吸附热相关的常数。

     - **BET等温线：**
       \[ \frac{P}{V(P_0 - P)} = \frac{1}{V_mC} + \frac{C - 1}{V_mC} \cdot \frac{P}{P_0} \]
       其中 \( P \) 是平衡压力，\( P_0 \) 是饱和蒸气压，\( V \) 是吸附量，\( V_m \) 是单层吸附量，\( C \) 是常数，与吸附质和吸附剂的相互作用相关。
